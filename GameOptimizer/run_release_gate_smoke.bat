...
%PYTHON_CMD% release_gate_evidence.py init --kind smoke --target "%TARGET%" --exe "%EXE%" > "%RUN_DIR_FILE%"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { $ErrorActionPreference='Stop'; try { & '.\%EXE%' '%TARGET%' --dry-run --max-runtime-seconds 60 2>&1 | Tee-Object -FilePath '%RUN_DIR%\logs\rg1_dry_run.log'; exit $LASTEXITCODE } catch { $_ | Out-String | Tee-Object -FilePath '%RUN_DIR%\logs\rg1_dry_run.log'; exit 1 } }"
set STEP_EXIT=%ERRORLEVEL%
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { $ErrorActionPreference='Stop'; try { & '.\%EXE%' '%TARGET%' --max-runtime-seconds 120 --thread-detail-log --thread-log-interval 4 2>&1 | Tee-Object -FilePath '%RUN_DIR%\logs\rg2_soft_apply.log'; exit $LASTEXITCODE } catch { $_ | Out-String | Tee-Object -FilePath '%RUN_DIR%\logs\rg2_soft_apply.log'; exit 1 } }"
set STEP_EXIT=%ERRORLEVEL%
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { $ErrorActionPreference='Stop'; try { & '.\%EXE%' '%TARGET%' --apply --latency-ping 8.8.8.8 --background-filter background_filter_example.txt --max-runtime-seconds 180 2>&1 | Tee-Object -FilePath '%RUN_DIR%\logs\rg3_apply.log'; exit $LASTEXITCODE } catch { $_ | Out-String | Tee-Object -FilePath '%RUN_DIR%\logs\rg3_apply.log'; exit 1 } }"
set STEP_EXIT=%ERRORLEVEL%
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { $ErrorActionPreference='Stop'; try { & '.\%EXE%' '%TARGET%' --dry-run --max-runtime-seconds 5 2>&1 | Tee-Object -FilePath '%RUN_DIR%\logs\rg4_timeout.log'; exit $LASTEXITCODE } catch { $_ | Out-String | Tee-Object -FilePath '%RUN_DIR%\logs\rg4_timeout.log'; exit 1 } }"
set STEP_EXIT=%ERRORLEVEL%
