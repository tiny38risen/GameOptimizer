...
When the candidate check passes, create the immutable evidence bundle:
...
3. `create_rc_evidence_bundle.py --target <target.exe> --regression-log <log>` writes `RC_CANDIDATE_PASS`.
