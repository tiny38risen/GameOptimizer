...
`StartupPipeline::run()` must keep core runtime component assembly, startup mutation, runtime service assembly, and runtime sensor activation in named helpers. The static gate rejects loss of those helper markers because startup execution is the boundary where constructed services begin mutating or observing runtime state.
...
SchedulerController affinity rollback context logging must remain context evidence only; release selftests must prove it does not create a second ApplyGuard rollback BLOCKER when paired with the ApplyGuard explicit rollback failure marker.
...
The release blocker list must keep Processor Group 1+ monitoring-only background restriction under `WARN`; the static gate rejects that marker under `BLOCKER`.

The release blocker list must keep Access Denied/access-boundary events with fallback evidence under `WARN`; the static gate rejects that marker under `BLOCKER`.

The release blocker list must keep unsupported IRQ affinity with monitoring-only evidence under `WARN`; the static gate rejects that marker under `BLOCKER`.
...
The release blocker list must keep DPC spike observation while IRQ mutation backend is unavailable under `WARN`; the static gate rejects that marker under `BLOCKER`.
