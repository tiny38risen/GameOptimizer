...
- `run_rc_gate.bat` is missing an `[RC-1]` through `[RC-9]` step marker or the markers are out of order.
...
- `StartupPipeline::run()` core runtime component/startup mutation/runtime service/runtime sensor helper markers are missing.
...
- SoftApply limitation marker is moved out of `WARN` or duplicated under `BLOCKER`.
